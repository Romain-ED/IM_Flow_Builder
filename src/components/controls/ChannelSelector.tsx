import { useSimulatorStore } from '../../store/simulatorStore'
import { CHANNEL_OPTIONS } from '../../channels/registry'

export function ChannelSelector() {
  const channel = useSimulatorStore((s) => s.channel)
  const setChannel = useSimulatorStore((s) => s.setChannel)

  return (
    <div className="flex flex-col gap-1.5">
      <span className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Channel</span>
      <div className="grid grid-cols-3 gap-1.5" role="radiogroup" aria-label="Channel">
        {CHANNEL_OPTIONS.map((opt) => (
          <button
            key={opt.id}
            type="button"
            role="radio"
            aria-checked={channel === opt.id}
            onClick={() => setChannel(opt.id)}
            className={`rounded-lg border px-2 py-2 text-[11.5px] font-medium text-center transition-colors cursor-pointer focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-1 ${
              channel === opt.id
                ? 'bg-slate-900 text-white border-slate-900'
                : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300'
            }`}
          >
            {opt.id.toUpperCase()}
          </button>
        ))}
      </div>
    </div>
  )
}
