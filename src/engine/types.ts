 ...
 * governs `node.actions` (rendered attached to the last message, like a
 * `suggested_replies` message) — inline interactive messages
 * (suggested_replies/list/input) stay individually tappable for as long as
 * they belong to the current node, independent of this outcome (see
 * `isMessageInteractive`).
 */
export type NodeOutcome =
  ...
  | { kind: 'terminal' }
  /** No `actions` and no auto-transition, but the node's messages still contain live interactive content (e.g. a boarding pass or carousel). */
  | { kind: 'idle' }
