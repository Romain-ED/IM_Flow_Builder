...
`beerlao.yaml` is a deliberate exception: it uses `otp`'s segmented code-entry UI on purpose, to demonstrate a user-typed one-time code with retry-on-wrong-code — a legitimate simulator extension for a story that doesn't map cleanly to WhatsApp's real Authentication Template. It still shows the honest capability-warning note explaining the real-world equivalent; it just isn't rewritten away like the other three.
